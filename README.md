...
# Com cobertura (gate: >= 90% sobre core/auth/analyzer/export/utils — .coveragerc)
...
A suite conta com **465 testes** cobrindo: análise de AST, métricas, padrões, pipeline, comparação, cache (disco e distribuído), OAuth, repositório, exportação, compressão, performance, alertas, IA e normalização de histórico. Coverage atual: **~91%** dos módulos de lógica (UI e `app.py` fora do gate — cobertos por smoke test manual).
